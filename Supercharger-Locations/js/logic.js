   ...
   // console.log(response); // Check the response data
    ...
    if (response && Array.isArray(response)) {
        response.forEach(function(site) {
            ...
            circleMarker.bindTooltip(`<strong>Place: </strong> ${site.name}<br><strong>Address: </strong> ${site.address.street}<br>}<strong>         </strong>${site.address.city<br>
                <strong>         </strong>${site.address.state}<strong>, </strong>${site.address.zip}<br><strong>Stall Count: </strong>${site.stallCount}`);
